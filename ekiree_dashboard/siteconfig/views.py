...
def EnterStudents(request):
    if request.method == "POST":
        ...
        if form.is_valid and temp_file == "text/plain; charset=us-ascii":
            submitted = import_students(
                request.FILES["file"]
            )  # runs the import_students function from import_students.py
            return render(
                request, "siteconfig/SuccessSubmit.html", {"submitted": submitted}
            )
        else:
            subprocess.Popen(
                ["rm", upload]
            )  # deletes files that get rejected so they can't be run
